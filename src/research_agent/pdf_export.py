...
class PdfBlock:
    ...
    ``kind`` is one of: ``h1``, ``h2``, ``h3``, ``bullet``, ``para``.
    """
...
def markdown_to_blocks(markdown: str) -> list[PdfBlock]:
    ...
    for raw in (markdown or "").splitlines():
        ...
        if bullet:
            blocks.append(PdfBlock("bullet", strip_inline_markdown(bullet.group(1))))
            continue
        ordered = re.match(r"^(\d+)[.)]\s+(.*)$", stripped)
        if ordered:
            blocks.append(PdfBlock("bullet", f"{ordered.group(1)}. {strip_inline_markdown(ordered.group(2))}"))
            continue
        blocks.append(PdfBlock("para", strip_inline_markdown(stripped)))
    return blocks
...
def render_pdf_bytes(
    ...
    *,
    ...
    for block in markdown_to_blocks(markdown):
        ...
        pdf.multi_cell(0, size * 0.55 + 2, text)
        if space_after:
            pdf.ln(space_after)
